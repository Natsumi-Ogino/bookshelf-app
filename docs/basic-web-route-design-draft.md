...
この文書は、要件シートのシート5「画面設計」、シート6「デザインUI」、シート7「機能要件」、提供されたBasic版Blade、2026年9月8日、9月15日および9月16日のコーチ回答を照合した設計案です。

画面遷移、成功メッセージ、自分のレビューへのいいね禁止、レビューコメント、バリデーションエラーメッセージ、テスト範囲、Basic版公開APIの主要仕様は確認済みです。公開APIの404・422レスポンス形式と成功時のHTTPステータスコードだけは追加回答を待っています。
...
1. HTTP 422を`message`と`errors`、HTTP 404を`error`のみとするか、404と422を同じJSON形式に統一するか。
2. 登録成功時201、一覧・詳細・更新成功時200、削除成功時204で確定してよいか。
